...
public:
  ...
  Matrix euclidean_distances_sq_gpu(const Matrix& X, const Matrix& Y)
  {
    tp_.tic("euclideanDistGPU");
    unsigned int N = X.rows();
    unsigned int K = Y.rows();

    // take to GPU
    Tensor Xt = EigenMatrixToTensor(X, open3d::core::Device("CUDA:0")).Reshape({ N, 1, D });
    Tensor Yt = EigenMatrixToTensor(Y, open3d::core::Device("CUDA:0")).Reshape({ 1, K, D });

    Tensor Diff = (Xt - Yt).Reshape({ N * K, 1, D });

    Tensor Dist = Diff.MatmulBatched(Diff.Transpose(1, 2));

    tp_.toc("euclideanDistGPU");

    return TensorToEigenMatrix<T>(Dist.Reshape({N, K}));
  }
